...
public class MenuScreen extends LemmingScreen {
    ...
    // Stores a button to put responses into
    private TextButton resBtn;
    ...
    public void menuVictory(int sheepHome, int sheepTotal, int score) {
        ...
        if(sheepHome >= sheepTotal) {
            // Show them victory message
            btn = new TextButton("VICTORY!", skin);
            table.add(btn).size(150,60).padBottom(5).row();
            
            // A reference to their score
            final int theirScore = score;
            
            // A reference to this
            final MenuScreen ms = this;
            
            // Add option to submit highscore
            btn = new TextButton("Submit Score", skin);
            btn.addListener(new ClickListener(){
                @Override
                public void clicked(InputEvent event, float x, float y) {
                    ms.submitScore(network.getLobby().mapName, theirScore);
                }
            });
            table.add(btn).size(150,60).padBottom(5).row();
        }
        ...
        // Button that shows info on stuff
        resBtn = new TextButton("", skin);
        table.add(resBtn).size(150,60).padBottom(5).row();
    }
    ...
    public void menuHighScores(String mapName) {
        ...
        // Button that shows info on stuff
        resBtn = new TextButton("Searching...", skin);
        table.add(resBtn).size(150,60).padBottom(5).row();
        ...
        Gdx.net.sendHttpRequest (httpGet, new HttpResponseListener() {
            ...
            public void handleHttpResponse(HttpResponse httpResponse) {
                ...
                if(json.has("failure") || !json.has("scores")) {
                    // No highscores!
                	resBtn.setText("No highscores found!");
                } else {
                	// Change message
                	resBtn.setText("Scores:");
            ...
            public void failed(Throwable t) {
                ...
                resBtn.setText("Failed to connect!");
            }
    ...
    public void submitScore(String mapName, int score) {
        // Setup the request
        HttpRequest httpGet = new HttpRequest(HttpMethods.GET);
        httpGet.setUrl(String.format(Constants.HIGHSCORE_SERVER_ADD, mapName, "Player", score));
        ...
        Gdx.net.sendHttpRequest (httpGet, new HttpResponseListener() {
            ...
            public void handleHttpResponse(HttpResponse httpResponse) {
                ...
                if(json.has("failure") || !json.has("scores")) {
                    // Failued to connect
                	resBtn.setText("Failed to update highscores!");
                } else if(json.has("added")) {
                    // Their score was added!
                	resBtn.setText("You got a new highscore!");
                } else {
                    // Nothing special
                	resBtn.setText("Your score wasn't good enough!");
                }
            }
